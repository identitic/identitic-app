...
/*     return FutureBuilder(
        ...
        builder: (_, AsyncSnapshot<List<Message>> snapshot) {
          ...
          return ListView.builder(
            reverse: true,
            shrinkWrap: true,
            itemCount: Provider.of<SocketsProvider>(context,listen: false).fakeMessages?.length ?? 10,
            physics: ClampingScrollPhysics(),
            padding: EdgeInsets.only(top: 15.0),
            itemBuilder: (_, int i) {
              final List<Message> list = List.from(Provider.of<SocketsProvider>(context,listen: false).fakeMessages.reversed);
  ...
  void _sendMessage(context) {
    ...
    Provider.of<SocketsProvider>(context,listen:false).fakeMessages.add(Message(
        content: textEditingController.text, senderID: "1", receiverID: "2"));
    textEditingController.text = '';
